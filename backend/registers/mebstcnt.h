...
public:
  ...
  const MEBSTCNT& operator= (const MEBSTCNT& other) {fBits = other.fBits; return* this;}
  bool operator== (const MEBSTCNT& other) const {return fBits.to_ulong() == other.fBits.to_ulong();}
  ...
  const std::bitset<4>& GetBits() const {return fBits;}
  void SetValue(Register::Value value) {fBits = value;}
  void SetBits(const std::bitset<4>& bits) {fBits = bits;}

  // MEBSTCNT specific parameters
  static const Register::Access Access = Register::R;
  static const Register::Address Address = 0x511b;
  static const Register::Type Type = Register::RCU;

  // MEBSTCNT specific members:
  // TODO: implement MEBSTCNT specific members
