...
namespace RcuRegisterMap
  ...
  const unsigned long L1_LATENCY               = 0x4006;    //32bit R/W
  const unsigned long L1_MSG_LATENCY           = 0x400a;    //32bit R/W

  //OD
  // CRAP wrong place I suppose...
  ...
  const unsigned long RCU_READ_MEMBLOCK       = 0x110000 | FEESVR_CMD_RCU;
  const unsigned long RCU_READ_RESULT         = 0x0e0000 | FEESVR_CMD_RCU;

  const unsigned long CE_CMD_TRAILER          = 0xdd330000 | 0x0007;

  const unsigned long RCU_WAIT_COMMAND        = 0x320000;
  ...
  const unsigned long  Result_MEM             = 0x2000;  //20bit  R/W     // The result of each ALTRO READ instruction is stored in this memory.
